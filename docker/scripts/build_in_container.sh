...
cd ${PROJECT_PATH} && rm -rf ${BUILD_FOLDER} && mkdir ${BUILD_FOLDER} && cd ${BUILD_FOLDER} && source "/var/emsdk/emsdk_env.sh" && cmake -DEMSCRIPTEN=1 -DCMAKE_BUILD_TYPE=Debug .. && make -j$(nproc)