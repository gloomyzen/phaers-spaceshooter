...
namespace {
	...
	struct Device {
		...
		SDL_Texture *MakeTexture(int width, int height) {
			SDL_Texture *texture = SDL_CreateTexture(Renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_TARGET, width,
													 height);
	...
	template<typename T>
	class InterpolatedFactorEquation {
	public:
		InterpolatedFactorEquation(const T &value0, const T &value1, const T &value2, const ImVec2 &v0,
								   const ImVec2 &v1, const ImVec2 &v2)
	...
	struct FixedPointTriangleRenderInfo {
		...
		CalculateFixedPointTriangleInfo(const ImVec2 &v1, const ImVec2 &v2, const ImVec2 &v3) {
	...
	void DrawTriangleWithColorFunction(const FixedPointTriangleRenderInfo &renderInfo,
	...
	void DrawTriangle(const ImDrawVert &v1, const ImDrawVert &v2, const ImDrawVert &v3, const Texture *texture) {
		...
		const auto key = std::make_tuple(
				std::make_tuple(static_cast<int>(std::round(v1.pos.x)) - renderInfo.MinX,
								static_cast<int>(std::round(v1.pos.y)) - renderInfo.MinY, v1.uv.x, v1.uv.y, v1.col),
				std::make_tuple(static_cast<int>(std::round(v2.pos.x)) - renderInfo.MinX,
								static_cast<int>(std::round(v2.pos.y)) - renderInfo.MinY, v2.uv.x, v2.uv.y, v2.col),
				std::make_tuple(static_cast<int>(std::round(v3.pos.x)) - renderInfo.MinX,
								static_cast<int>(std::round(v3.pos.y)) - renderInfo.MinY, v3.uv.x, v3.uv.y, v3.col));
		...
		const InterpolatedFactorEquation<Color> shadeColor(Color(v1.col), Color(v2.col), Color(v3.col), v1.pos, v2.pos,
														   v3.pos);
	...
	void DrawUniformColorTriangle(const ImDrawVert &v1, const ImDrawVert &v2, const ImDrawVert &v3) {
		...
		const auto key = std::make_tuple(v1.col,
										 static_cast<int>(std::round(v1.pos.x)) - renderInfo.MinX,
	...
	void
	DrawRectangle(const Rect &bounding, SDL_Texture *texture, int textureWidth, int textureHeight, const Color &color,
				  bool doHorizontalFlip, bool doVerticalFlip) {
		...
		if (bounding.UsesOnlyColor()) {
			...
			const SDL_RendererFlip flip = static_cast<SDL_RendererFlip>((doHorizontalFlip ? SDL_FLIP_HORIZONTAL : 0) |
																		(doVerticalFlip ? SDL_FLIP_VERTICAL : 0));

			SDL_SetTextureColorMod(texture, static_cast<uint8_t>(color.R * 255), static_cast<uint8_t>(color.G * 255),
								   static_cast<uint8_t>(color.B * 255));
			SDL_RenderCopyEx(CurrentDevice->Renderer, texture, &source, &destination, 0.0, nullptr, flip);
		}
	}

	void DrawRectangle(const Rect &bounding, const Texture *texture, const Color &color, bool doHorizontalFlip,
					   bool doVerticalFlip) {
		DrawRectangle(bounding, texture->Source, texture->Surface->w, texture->Surface->h, color, doHorizontalFlip,
					  doVerticalFlip);
	}

	void DrawRectangle(const Rect &bounding, SDL_Texture *texture, const Color &color, bool doHorizontalFlip,
					   bool doVerticalFlip) {
		int width, height;
...
#define TEXT(v) ImVec4(0.860f, 0.930f, 0.890f, v)
	...
	void Initialize(SDL_Renderer *renderer, int windowWidth, int windowHeight) {
		...
		SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(pixels, width, height, 32, 4 * width, rmask, gmask, bmask,
														amask);
				...
				if (drawCommand->UserCallback) {
					...
					for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3) {
						...
						if (i + 6 <= drawCommand->ElemCount) {
							...
							const bool isUniformColor =
									isTriangleUniformColor && v2.col == v3.col && v3.col == v4.col && v4.col == v5.col;
							...
							if (isUniformColor
								...
								if (isWrappedTexture) {
									DrawRectangle(bounding, static_cast<const Texture *>(drawCommand->TextureId),
												  Color(v0.col), doHorizontalFlip, doVerticalFlip);
								} else {
									DrawRectangle(bounding, static_cast<SDL_Texture *>(drawCommand->TextureId),
												  Color(v0.col), doHorizontalFlip, doVerticalFlip);
								}
